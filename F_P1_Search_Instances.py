...
class P1_Search_Instances():
	def __init__(self):
		self.instances_list: list[P1_Instance] = []
		self.total_unit = 0
	...
	def __call__(self, n_instance: int) -> P1_Instance:
		...
		for path, _, files in os.walk(self.path_instances):
			for file in files:
				if file.startswith("instancia") and file.endswith(".txt"):
					inst = P1_Instance(os.path.join(path, file))
					self.instances_list.append(inst)
					self.total_unit += inst.get_number_of_items() * inst.get_number_of_picks()
